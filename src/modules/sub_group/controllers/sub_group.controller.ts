...
export class SubGroupController {
    ...
    async findAll(req: Request, res: Response, _next: NextFunction) {        
        ...
        const {
            data,
            total: totalItems
        } = await this.subGroupSvc.findAll(offset, Number(perPage));
    ...
    async findOneById(req: Request, res: Response, _next: NextFunction) {
        ...
        const result = await this.subGroupSvc.findOneById(Number(id));

        if (!result) {
            res.status(404).json({'message': 'Sub Grupo não encontrado!'}).end();
        }
