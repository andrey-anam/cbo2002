...
export class BigGroupController {
    ...
    async findAll(req: Request, res: Response, _next: NextFunction) {        
        ...
        const {
            data,
            total: totalItems
        } = await this.bigGroupSvc.findAll(offset, Number(perPage));
    ...
    async findOneById(req: Request, res: Response, _next: NextFunction) {
        ...
        const result = await this.bigGroupSvc.findOneById(Number(id));

        if (!result) {
            res.status(404).json({'message': 'Grande Grupo não encontrado!'}).end();
        }
