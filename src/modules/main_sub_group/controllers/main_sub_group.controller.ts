...
export class MainSubGroupController {
    ...
    async findAll(req: Request, res: Response, _next: NextFunction) {        
        ...
        const {
            data,
            total: totalItems
        } = await this.mainSubGroupSvc.findAll(offset, Number(perPage));
    ...
    async findOneById(req: Request, res: Response, _next: NextFunction) {
        ...
        const result = await this.mainSubGroupSvc.findOneById(Number(id));

        if (!result) {
            res.status(404).json({'message': 'Sub Grupo Principal não encontrado!'}).end();
        }
